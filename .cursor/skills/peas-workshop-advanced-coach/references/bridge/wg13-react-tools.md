...
- Add `get_identity()` by upgrading the WG-12 identity prompt:
  ...
  - Replace or expand the old short `system_text` so it includes the WG-13 standard identity text:
    - `你是課堂程式助教，並請使用繁體中文。`
    - `【解題方式】`
    - `write_file`
    - `exec`
...
- `get_identity`, `add_numbers`, `TOOLS`, `_stream_model_response`, and `run_react_turn` exist.
- `get_identity()` includes `【解題方式】`, `write_file`, `exec`, `uv run python`, `【依賴管理】`, `uv add`, and `不要用 pip install`.
...
must_include: TOOLS = [add_numbers], 解題方式, write_file, exec, uv run python, 依賴管理, uv add, no pip install
forbidden: file/shell tools, JSONL, token budget, agent_core.py
verify: required WG-13 symbols exist including TOOLS; get_identity contains standard WG-13 identity text; main.py parses
